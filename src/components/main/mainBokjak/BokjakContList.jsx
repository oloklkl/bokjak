...
const BokjakContList = () => {
    ...
    const movie = movies;
    const bokjakItems = bokjakData;

    // combinedContent: 영화 데이터와 복작 데이터의 타이틀, 시간 결합
    const combinedContent = movie
        .filter((content) => content.logoImage)
        .slice(0, 10)
        .map((content, index) => {
            const bokjakTitle =
                bokjakItems[index]?.bokjak_title ||
                'Default Title';
            const bokjakTime =
                bokjakItems[index]?.bokjak_time ||
                'Default Time';
            const bokjakPeople =
                bokjakItems[index]?.expected_participants ||
                'Default Time';
