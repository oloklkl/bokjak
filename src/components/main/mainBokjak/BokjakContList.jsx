...
const BokjakContList = () => {
    ...
    const combinedContent = movie.map((content, index) => {
        ...
        const bokjakPeople = bokjakItems[index]?.expected_participants || 'Default Time';
        ...
        <BokjakListWrap>
            <BokjakHeader>
                <div className="quesCont">
                    <h2>🐣 모여봐요 복작 </h2>
                    <IconButton className="border" icon={<QuestionMark size={18} />} text="smiley" />
                    <div className="msgBox">
                        <div className="textarea">
                            ‘모여보기 복작’은 친구들과 실시간으로 영화를 보며 채팅으로 소통하는 특별한 경험입니다.{' '}
                            <br />
                            🎬혼자서 느끼기엔 아쉬운 순간들, 이제 함께 즐기며 더 특별하게 만들어보세요! 💬✨ 멀리 있어도
                            함께하는 그 순간, ‘모여보기 복작’에서 실시간 채팅으로 감상을 나누며 경험해보세요.
                        </div>
                    </div>
                </div>
                <Link>
            ...
            {isModalOpen && selectedContent && <BokjakModal content={selectedContent} closeModal={closeModal} />}
        </BokjakListWrap>
    );
};
