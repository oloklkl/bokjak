...
const ThumbnailContList = ({ title, targetGenreId, typeFilter }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            // 장르 필터링 적용
            if (targetGenreId) {
                combinedContent = combinedContent.filter((content) => content.genre_ids.includes(targetGenreId));
            }
                ...
                <ThumbnailList>
                    ...
                    >
                        {filteredContent
                            ...
                            .map((content) => (
                                <SwiperSlide className="swiper-slide" key={`${content.type}-${content.id}`}>
                                    <Link to={`/${content.type}/${content.id}`} state={{ previousLocation: location }}>
                                        <ThumbnailCard
                                            content={content}
                                            onClick={() => {
                                                // 데스크탑이 아닐 때만 모달을 여는 함수 호출
                                                showDetailModal(content.type, content.id, content.genre_ids);
                                            }}
                                        />
                                    </Link>
                                </SwiperSlide>
